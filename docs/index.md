...
短期先建立乾淨、可回溯的 supervised anomaly detection baseline：

```mermaid
flowchart LR
    img[工業瑕疵影像] --> enc[DINOv3 frozen encoder]
    enc --> cls[CLS token / patch tokens]
    cls --> head[Linear / MLP 異常分類頭]
    head --> score[異常分數]
    score --> metrics[AUROC / AUPRC / F1]

    split[VisA supervised split] --> train[只訓練分類頭]
    train --> head
```
...
| 第一個 baseline | `EXP-001` 已完成：overall 與 per-category image-level 表格已產出 | MLP head 比較與 failure-case inspection |
